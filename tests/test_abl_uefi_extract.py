...
class ExtractTest(unittest.TestCase):
    ...
    def test_dump_contract_emits_flat_bin_files_for_audit_glob(self):
        with tempfile.TemporaryDirectory() as directory:
            root = Path(directory)
            image = root / "abl.bin"
            output = root / "manifest.json"
            dump = root / "dump"
            image.write_bytes(self._image())
            self.assertEqual(
                extract.main([
                    "--image", str(image), "--dump-dir", str(dump),
                    "--output", str(output),
                ]),
                0,
            )
            ...
            self.assertEqual(manifest["schema"], extract.SCHEMA)
            self.assertEqual(manifest["target"]["partition"], "abl")
            self.assertEqual(
            ...
            self.assertEqual(
                ...
                extract.CAPTURE_MANIFEST_SHA256,
            )
            self.assertEqual(manifest["device_binding"]["status"],
                             "NOT_APPLICABLE")
            self.assertEqual(manifest["command_status"],
                             "REDACTED_REPRODUCTION_TEMPLATE")
            self.assertIn("--experiment-id 029A-bank-relation-audit-over-abl",
                          manifest["commands"][1])
            self.assertIn("--source-manifest <029A extraction manifest>",
                          manifest["commands"][1])
            files = sorted(dump.iterdir())
            self.assertTrue(files)
            self.assertTrue(all(path.suffix == ".bin" for path in files))
            self.assertEqual(
                sorted(path.name for path in files),
                sorted(item["dump_name"]
                       for item in manifest["extracted"].values()),
            )
            self.assertTrue(
                any(name.endswith("/Loader/PE32")
                    for name in manifest["extracted"])
            )


class DumpFilenameTest(unittest.TestCase):
    def test_dump_filename_is_flat_suffixed_and_deterministic(self):
        self.assertEqual(
            extract.dump_filename("abl/file@0x48/lzma/Loader/PE32"),
            "abl_file@0x48_lzma_Loader_PE32.bin",
        )
        self.assertEqual(extract.dump_filename("already.bin"), "already.bin")
        self.assertNotIn("/", extract.dump_filename("../unsafe/name"))
