...
class MaterialisationAuditTests(unittest.TestCase):
    ...
    def test_a_stored_word_is_counted(self):
        ...
        self.assertFalse(record["single_instruction_paths_all_absent"])

    def test_all_paths_absent_on_an_empty_image(self):
        record = self._audit({"synthetic.bin": bytes(4096)})["0x00003333"]
        self.assertTrue(record["single_instruction_paths_all_absent"])

    def test_wide_move_is_only_excluded_when_a_halfword_is_missing(self):
        # MOVZ W0,#0x3333 alone leaves the high halfword unaccounted for
        movz = struct.pack("<I", 0x52800000 | (0x3333 << 5))
        record = self._audit({"synthetic.bin": movz * 4})["0x00003333"]
        self.assertFalse(record["wide_move_pair_not_excluded"])

    def test_wide_move_not_excluded_when_both_halfwords_appear(self):
        ...
        self.assertTrue(record["wide_move_pair_not_excluded"])
...
class ExactImageTests(unittest.TestCase):
    ...
    def test_manifest_uses_the_suffix_experiment_id_and_static_dod_fields(self):
        self.assertEqual(self.manifest["experiment_id"], inv.EXPERIMENT_ID)
        ...
        self.assertIn("019a", dod["commands"]["command"])
        self.assertIn("default private", dod["commands"]["reason"])
        self.assertIn("not an executed-command receipt", dod["commands"]["reason"])
    ...
    def test_qhs_mc_0x404_value_is_absent_from_every_single_instruction_path(self):
        record = next(
            r for r in self.manifest["ranked_value_materialisation"] if r["value"] == "0x00003333"
        )
        self.assertTrue(record["single_instruction_paths_all_absent"])

    def test_materialisation_claim_does_not_promote_observed_literals_to_absence(self):
        proved = " ".join(self.manifest["claims"]["PROVED"])
        self.assertIn("only 0x00003333 is absent", proved)
        self.assertNotIn("Every ranked live value except 0x00001111 is absent", proved)

    def test_claims_bound_pair_shape_without_proving_a_consumer(self):
        claims = self.manifest["claims"]
        self.assertIn("candidate", " ".join(claims["PROVED"]).lower())
        self.assertIn("HYPOTHESIS", claims)
        self.assertIn("consumer", " ".join(claims["UNKNOWN"]).lower())
        self.assertIn("absolute-address candidate", " ".join(claims["REFUTED"]))
