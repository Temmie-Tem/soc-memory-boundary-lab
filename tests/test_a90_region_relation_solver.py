...
class AnalyseTest(unittest.TestCase):
    ...
    def test_end_to_end_resolves_every_gate(self):
        ...
        self.assertEqual(result["experiment_id"], solver.EXPERIMENT_ID)
        self.assertEqual(result["summary_count"], len(result["differences"]))
        self.assertEqual(
            result["unique_difference_count"],
            len({entry["difference"] for entry in result["differences"]}),
        )
    ...
    def test_blind_pagemap_keeps_physical_base_support_within_model(self):
        text = self._phase_text()
        text["phaseA"] += "\n" + json.dumps({
            "schema": "a90_region_probe_r_v1",
            "type": "pa_provenance",
            "source": "pagemap",
            "status": "BLIND",
        })
        result = solver.analyse(text)
        self.assertEqual(result["pa_provenance"]["pagemap_status"], "BLIND")
        self.assertEqual(
            result["physical_provenance"]["classification"],
            "SUPPORTED_WITHIN_MODEL",
        )
        self.assertEqual(
            result["pa_provenance"]["base_offset"]["classification"],
            "SUPPORTED_WITHIN_MODEL",
        )
        self.assertIn("contiguous qsecom", result["physical_provenance"]["reason"])

    def test_blind_pagemap_does_not_publish_reported_contiguity_as_effective(self):
        text = self._phase_text()
        text["phaseA"] += "\n" + json.dumps({
            "schema": "a90_region_probe_r_v1",
            "type": "pa_provenance",
        ...
        result = solver.analyse(text)
        record = result["pa_provenance"]["pagemap"][-1]
...
class PublishedManifestTest(unittest.TestCase):
    def test_published_manifest_records_summary_count_and_blind_base_scope(self):
        path = Path(__file__).resolve().parents[1] / (
            "evidence/manifests/023R-repaired-region-bank-relation-20260826-01.manifest.json"
        )
        document = json.loads(path.read_text(encoding="utf-8"))
        self.assertEqual(document["experiment_id"], solver.EXPERIMENT_ID)
        self.assertEqual(document["summary_count"], 66)
        self.assertEqual(document["unique_difference_count"], 58)
        self.assertEqual(document["physical_provenance"]["pagemap_status"], "BLIND")
        self.assertEqual(document["pa_provenance"]["pagemap_status"], "BLIND")
        self.assertEqual(
            document["physical_provenance"]["classification"],
            "SUPPORTED_WITHIN_MODEL",
        )
        self.assertEqual(
            document["pa_provenance"]["base_offset"]["classification"],
            "SUPPORTED_WITHIN_MODEL",
        )
        for record in document["pa_provenance"]["pagemap"]:
            self.assertEqual(record["effective_contiguity"], "UNKNOWN")
            self.assertNotEqual(record.get("contiguous"), True)
        self.assertEqual(
            document["provenance"]["context_sha256"],
            solver.RETAINED_CONTEXT_SHA256,
        )
        self.assertEqual(
            document["provenance"]["retained_context_sha256"],
            solver.RETAINED_CONTEXT_SHA256,
        )
        self.assertEqual(
            document["provenance"]["retained_context"]["status"],
            "SUPPORTED_RETAINED_CONTEXT",
        )
        self.assertEqual(
            document["coordinate_scope"]["pa24_contribution"],
            "allocation-offset/model coordinates",
        )
        for section in ("new_bit", "rank", "kernel"):
            self.assertEqual(
                document[section]["coordinate_scope"],
                "allocation-offset/model coordinates",
            )
            self.assertEqual(
                document[section]["physical_mapping_classification"],
                "SUPPORTED_WITHIN_MODEL",
            )
        self.assertIn("pagemap is BLIND", " ".join(document["claims"]["SUPPORTED"]))
