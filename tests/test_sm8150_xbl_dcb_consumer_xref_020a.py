...
class ExactImageTests(unittest.TestCase):
    ...
    def test_manifest_uses_the_suffix_experiment_id_and_static_dod_fields(self):
        self.assertEqual(self.manifest["experiment_id"], xref.EXPERIMENT_ID)
        ...
        self.assertIn("020a", dod["commands"]["command"])

    def test_claims_keep_segment_identity_supported_and_nonconsumption_hypothetical(self):
        claims = self.manifest["claims"]
        self.assertIn("SUPPORTED", claims)
        self.assertIn("candidate DDR-driver", " ".join(claims["SUPPORTED"]))
        self.assertIn("HYPOTHESIS", claims)
        self.assertIn("does not consume", " ".join(claims["HYPOTHESIS"]))
        self.assertIn("indirect", " ".join(claims["REFUTED"]).lower())
        self.assertIn("outside Experiment 018's bounded", " ".join(claims["REFUTED"]))
        self.assertIn("writer absence", " ".join(claims["UNKNOWN"]))
        self.assertIn("passed or cached pointers", " ".join(claims["UNKNOWN"]))
        self.assertIn("Recognized local", self.manifest["section_directory_reader_scope"])
