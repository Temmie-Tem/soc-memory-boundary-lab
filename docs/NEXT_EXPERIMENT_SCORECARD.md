...
|---:|---|---|---|---|
| 1 | 020E static-slot consumer census | Census the six 020D static slots for bounded direct loads/stores/consumers, preserving register/value semantics and refusing to infer runtime MMIO from static VAs. | Host-only exact-XBL code/data cross-reference; strict direct definitions and static-slot interval matching; indirect paths, runtime execution/currentness/type and physical meaning stay `UNKNOWN`; no device/MMIO/controller write. | `PRIMARY NEXT; BOUNDED ONLY` |
...
## Verification 020B, 020C and 020D completed results
...
The next candidate, 020E, is deliberately non-overlapping: census those six
static slots for bounded direct consumers/writers without promoting static VAs
to runtime MMIO or DRAM destinations.
