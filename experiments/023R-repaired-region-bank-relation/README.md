# Experiment 023R — repaired second-allocation bank relation and its uniqueness

## Why this experiment exists

Experiment 023 measured the DRAM bank relation in a second allocation/offset
domain and
...
allocation. The retained experiment record describes a TWRP recovery context;
this host analysis does not provide a fresh recovery-state receipt. The
operating point was already the one Experiment 014 recorded and was not
changed.

Conceptual Experiment 015 (a controlled normal-RAM alias) and Experiment 016
(protected-boundary reach) remain `NOT ELIGIBLE`. 023R does not observe or
prove a global absence of aliasing, transform mutation, protected reach, or
bypass; those questions remain `UNKNOWN` outside the measured read-only
allocation/model scope.
...
The extra `ISB` and the other protocol differences mean that 023's
non-conflicting differences near zero cannot be assigned to one causal change;
the timed quantity was not the same quantity as 014's. The protocol
non-comparability is proved, while the cause of each magnitude difference is
not isolated. `measure_reopen_once`, `read_cntvct` and `load_barrier` are now
...
- Five phases, **66 measurement summaries over 58 unique differences**: 34
  measured conflicting, 32 measured non-conflicting. Repeated summaries are
  retained; the unique-difference count is reported separately.
...
circular with the PA24 claim it supports. Its result is
`SUPPORTED_WITHIN_MODEL`, not a direct physical-page proof: `/proc/self/pagemap`
is `BLIND` for all five phases. Under the assumptions of a contiguous
`qsecom` allocation in the declared carveout and the Experiment 014 relation
over PA13..PA23, the unique offset solve supports base `0xa6000000`; the
region is distinct from Experiment 014's `0xf0400000` within that model.

## Allocation-offset bit 24 is resolved by exhaustive discrimination

Eight differences were chosen so that each of the eight possible 3-bit
contributions for allocation-offset/model bit 24 is the unique contribution
making exactly one of them conflict. All eight were measured.
...
identified the same value, since `0x1020000` is model bit 24 xor PA17 and a
conflict forces the model bit-24 contribution to match PA17. What one witness
cannot do is notice being wrong.
...
7 of the 9 dimensions a rank-three kernel over model bits PA13..PA24 requires, leaving
...
One representative of the unique kernel, in Experiment 014's model basis
(`model_bit24` denotes allocation-offset bit 24, not an observed physical PA):

```text
b0 = PA13 xor PA16 xor PA18 xor PA19 xor PA20 xor PA23
b1 = PA14 xor PA16 xor PA17 xor PA18 xor PA21 xor PA23 xor model_bit24
b2 = PA15 xor PA17 xor PA18 xor PA19 xor PA22 xor model_bit24
```
...
`PROVED` in allocation-offset/model coordinates: the four protocol repairs; a
class separation whose empty band contains both of Experiment 014's reported
bounds; **66 measurement summaries over 58 unique differences**; the
allocation-offset/model bit-24 contribution `0b110` by exhaustive
discrimination over all eight candidates with 18 held-out agreements; a
9-dimensional kernel span with exactly one
consistent kernel; model rank 3 with ranks 1, 2, 4 and 5 refuted; and the bank
relation holding in the independently measured allocation/offset domain.

`SUPPORTED_WITHIN_MODEL`: mapping those allocation-offset/model results to
physical PA24, rank 3, base `0xa6000000`, and a second physical region is
supported only under the contiguous-`qsecom` and Experiment-014-relation
assumptions. All five pagemap summaries are `BLIND`, so this is not direct
physical-address provenance.
...
`UNKNOWN`: the direct physical page identity (pagemap is `BLIND`); PA25 and
above, which a 32 MiB allocation cannot vary; behaviour in any region beyond
the two measured; the mapping from this relation to named controller registers;
whether any transform is mutable or reachable. The `/proc/self/pagemap` route
to physical addresses is `BLIND`, not merely unused.

Current classification is unchanged: `CLASS C (TRANSFORM ONLY)` /
`NO_BOUNDARY_BYPASS_OBSERVED` within this read-only observation. 023R
strengthens the model-coordinate transform description and its provenance; it
does not observe or prove global absence of alias, mutation, protected reach,
or bypass. Those boundaries remain `UNKNOWN`, and Experiments 015 and 016
remain `NOT ELIGIBLE`.
...
The reproduction command is a `REDACTED_REPRODUCTION_TEMPLATE`: private raw
and output paths are redacted, while the exact logical inputs are pinned by
their published hashes; it is not an executed-command receipt.

- Probe SHA-256:
  `31fa568ef1bf9257cbc3ba555dbda66458e6318f113dd4a4dca4c5ac739b4377`
  (historical producer; the current verifier source is independently hashed).
- Current verifier probe-source SHA-256:
  `2dbc81ef7595d30f627df8d28d24e21603d8c1c174074e85593b9fe8df5569e9`.
- Historical producer solver SHA-256 (raw result producer):
  `5f8d1f346a6d46f6b990cf069062b5fd79cad15a67227bffa061a70059cfadea`.
- Current verifier solver SHA-256: `036e530370f9118e873cfa7690111a3aa9502f1bcb0f9188bf1695dfa8fb4ed5`.
- Current verifier focused-test SHA-256: `e97c87649e3f8eb0632fe8b3dc8fe155a808d2f837a0d8110dae014c37e35fbe`.
- Historical producer manifest SHA-256 before this claim/provenance repair:
  `68c0b8c0e6952f72b094df44b0951fff324e2eb6d6e9def86d6bf88c72a7738b`.
- Repaired public manifest SHA-256: `5c3e14ff898c109de740d98260d974bca10751000f85977775cd467ac74aac2e`.
- Focused result: 35 tests pass. Full repository discovery is not rerun here.
...
- Retained context `context.txt` is pinned in public provenance by SHA-256
  `2733587759119ec48558a3a039da200fa4740033d7043e706f0e8919c1a94f3b` and is
  classified `SUPPORTED_RETAINED_CONTEXT`; the private path is not published.
