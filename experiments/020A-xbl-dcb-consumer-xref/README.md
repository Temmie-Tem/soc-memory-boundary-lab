...
target match. Experiment 019A then recorded candidate pair arrays with a
base-relative key domain; whether the boot chain programs registers from them
remains `UNKNOWN`.
...
`SUPPORTED`: the largest RWE **candidate DDR-driver** segment is at VA
`0x9fc00000`, file offset `0x86fd0`, and is 2,359,296 bytes by size/content
inference. It contains **zero** file-backed 64-bit values inside the SoC
control aperture `0x09000000..0x0a000000`, and **zero** equal to a ranked MC
base. The segment identity is not a symbol proof.

`PROVED` for the pinned setter: the candidate segment's controller base
pointers are written by a five-store setter at `[0x9fc06410,0x9fc0643c)`; its
DDR-driver interpretation remains `SUPPORTED`:
...
stores into globals in the candidate segment come from a register and 59 write
`XZR`; calling those globals DDR-driver state remains conditional on the
`SUPPORTED` segment interpretation.

`REFUTED`: a writer using the `SUPPORTED` candidate segment, runtime base
arguments, register-offset stores, or computed-address stores is outside
Experiment 018's bounded constant-terminating models. Therefore Experiment
018's negative cannot establish writer absence or a globally null evidentiary
result. Global evidentiary weight and writer absence remain `UNKNOWN`.

`UNKNOWN`: the runtime origin of the base arguments, and therefore the absolute
address any store in the candidate segment reaches; which register-offset store,
if any, consumes a DCB table; the implicit base of DCB sections 10, 11 and 12;
register semantics; the relation to the Experiment 014 GF(2) bank relation;
post-boot writability; alias; boundary bypass.
...
The AOP check searches only exact stored 32-bit literals and the recognized
local directory-reader pair. `PROVED` within those encodings: AOP contains
**no** literal equal to any of the four ranked MC bases, in any segment. Its
only DCB directory-read pairs are two isolated sections, 2 and 21, without the
consecutive-index run that separates the XBL dispatcher from coincidence.
Computed addresses, received pointers, and alternate encodings remain
`UNKNOWN`.

`REFUTED` for the direct literal and local consecutive-directory-reader model:
AOP contains no ranked-base literal and no consecutive directory-reader run.
`UNKNOWN`: whether AOP reaches the tables or controller instances indirectly,
through a pointer it receives rather than a literal it holds.

Read with the section-consumer result below, `HYPOTHESIS` (bounded to this
static evidence): the economical reading is that the exact XBL does not consume
the DCB base-relative candidate arrays at all. This is not a global consumer or absence
conclusion. The consumers it does contain compute a checksum and check bounds.
`UNKNOWN`: whether the consumer lives in another image or outside the bounded
models; `abl--sdd8.bin` is also ELF32 ARM and is not covered here, and neither
is the SHRM co-processor.
...
Under the recognized local directory-pair model, a DCB consumer reads a
directory slot as a pair of `LDRH` at `0x0c + index*4` and `+2` from one base
register. Scanning for that pair finds a contiguous run in `xbl--sdb1.bin` at `0x1485f0f8`, `0x1485f13c`, `0x1485f17c`,
...
Three bounded observations now line up. Experiment 019A records candidate DCB
pair arrays with base-relative offsets; if consumed as programming data, a
consumer would write through a register offset, which Stage 1A does not count.
The candidate DDR-driver segment's bases are runtime arguments held in
zero-initialised globals, so they are absent from the image as constants. None
of this proves that the arrays are consumed by this image.

Taken together these do not show that a writer exists. They show only that a
writer using the bounded constant-terminating idioms would not be represented
by Experiment 018's search, which is a different statement from global writer
absence. The ranked-candidate negatives from Experiments 014, 017 and 019A are
unaffected; what changes is how much additional weight Experiment 018 adds to
them, and that global evidentiary weight remains `UNKNOWN`.
...
`0x9fc00000` segment is `SUPPORTED` as a DDR-driver candidate by size and
content, which is an inference, not a symbol proof.
...
python3 -m unittest -v tests.test_sm8150_xbl_dcb_consumer_xref_020a
```

The reproduction command is a `REDACTED_REPRODUCTION_TEMPLATE`: private input
and output paths are redacted while the exact firmware hashes are pinned; it
is not an executed-command receipt.
...
- Historical producer focused-test SHA-256 (suffix test):
  `2212f7f7d57995f85ef9cc6d1dbfd418ec37a8aa0979d88b4a177a2adecf0ec4`.
...
- Current verifier tool SHA-256:
  `4d81d7f7aea9d4aaeb56c3fffc784538aaa02818c9cd3c97d34d04aa945c256d`.
- Current verifier focused-test SHA-256:
  `aa519e33b3fc06a51ae879bfaefcdf18dc7c6ff903f152f465be6163bab826b1`.
- Repaired public manifest SHA-256: `ed6cd5e988cbf5af67f8fd88a949b90b7169967265e44631b3acd23c14719572`.
- Focused result: 58 tests pass. Full repository discovery is not rerun here.
