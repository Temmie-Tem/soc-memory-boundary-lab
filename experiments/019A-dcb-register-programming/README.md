# Experiment 019A — DCB pair-array inventory
...
`PROVED`: the `xbl_config` DCB contains **28 candidate `(key, value)` pair
arrays** across the four DCB blocks that pass a strict syntactic acceptance
test: at least four entries, every key 4-byte aligned, keys strictly
ascending, and an all-zero terminator occupying the final eight bytes. This
shape alone does not prove that the arrays are register-programming tables.

`PROVED`: the accepted candidate arrays have separate **absolute-key** and
**base-relative-offset** domains; the relative arrays contain no base field.
That representation permits an absolute target to be absent as a firmware
literal, but it does not identify a consumer or establish register-programming
semantics. Consumer and register identity are `UNKNOWN`.

`REFUTED` within the parsed absolute-key, direct-stored-value and
base-relative-value models: the DCB does not establish programming of the
twelve ranked absolute MC targets. No absolute candidate reaches any ranked
base, and the only base-relative candidate carrying the ranked offsets does not
carry the measured values. This is not a global writer or consumer refutation.

`HYPOTHESIS`: the accepted pair arrays may be consumed as data-driven
controller programming, but this inventory does not establish a consumer or
register identity.

`UNKNOWN`: the implicit base of every base-relative array; whether the accepted
arrays are consumed as controller-programming data; whether any ranked value is
computed rather than stored; the identity and semantics of any consumer or
register writer; whether DDR training firmware outside these nine partitions
writes them; the relation to the Experiment 014 GF(2) bank relation; post-boot
writability; alias; bypass.
...
recorded as `NOT_A_CANDIDATE_PAIR_ARRAY` rather than promoted. Section 16 is
the Experiment-012 SHRM snapshot list and is not a programming table.
...
`0x09360000`, `0x093e0000`) is reached by any absolute-key candidate array.
...
Sections 10, 11 and 12 carry the configuration-dependent fields in the
candidate arrays. Section 7 is effectively fixed: one key of twenty-two
varies.

## The ranked offsets

Exactly one accepted candidate array contains the ranked offsets, and it is
section 7:
...
`+0x404 = 0x00003333`. Section 7 holds `0x10000000` at both. `REFUTED` for
the direct-value comparison: this candidate array does not establish storage
of those measured values. Its implicit base and runtime consumer are
`UNKNOWN`; the four-instance shape is suggestive but is not evidence of
identity.
...
It proves only that the DCB contains candidate pair arrays with the recorded
domains and values; it does not establish that a controller consumes them or
which registers they would reach. The parsed model is negative for the ranked
targets. Combined with Experiment 017 — where the only confirmed path touching
those addresses is a read-only snapshot copier — and Experiment 018's four
negative writer models, four independent methods have now failed on the same
twelve addresses, but a writer outside these models remains `UNKNOWN`.

`HYPOTHESIS` (bounded to the candidate-array and materialisation evidence): the
parsimonious reading is that the ranked candidate set, which Verification 012
itself published as `UNKNOWN` for every row after rejecting the comparative
Qualcomm label, may not be where the Experiment 014 bank relation lives.
Hardwired reset defaults and wrong-address remain jointly consistent with all
current evidence and are not separated here; the writer and consumer identity
remain `UNKNOWN`.

## Cheapest next discriminator

Recover the implicit bases of sections 10, 11 and 12 by locating consumers of
their candidate arrays in XBL, which must read the section directory at
`0x0c + index*4` and pair each offset with a base register. Those three arrays
hold the configuration-dependent fields and span offsets `0x10`..`0xf48`, a
plausible controller window.
...
python3 -m unittest -v tests.test_sm8150_dcb_register_program_inventory_019a
```

The reproduction command is a `REDACTED_REPRODUCTION_TEMPLATE`: the default
private firmware directory and output path are redacted while the exact
firmware inputs are pinned by their published hashes; it is not an
...
- Historical producer focused-test SHA-256 (suffix test):
  `e85ce6581bc1bb5b9853fa3ab1ee867a95af3fd100930f460135388e48bd5030`.
...
- Current verifier tool SHA-256:
  `beeda571fd9f696cfc6e7a456c5a95205871a80950ae6249811d7400a609aab7`.
- Current verifier focused-test SHA-256:
  `c558fc728138fe8d9ac3b325a4cfd717aec9536be7ceee22b19e21e1ab94b743`.
- Repaired public manifest SHA-256: `54ec8762ec8c437cd4b29690fc37376892608f3ed5e53d2eec0a2f56009baea1`.
- Focused result: 43 tests pass. Full repository discovery is not rerun here.
