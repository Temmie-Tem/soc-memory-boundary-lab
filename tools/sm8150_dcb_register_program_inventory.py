...
def classify_section(body: bytes) -> dict:
    """Accept a section as a candidate pair array only under a strict test.
...
def logical_immediates_32() -> dict[int, tuple[int, int, int]]:
    ...
    alias of ``ORR Wd,WZR,#imm`` and is audited as a separate logical-immediate
    path.
    """
...
def _wide_move_halfword_encodings(value: int) -> dict[str, int]:
...
def _decode_wide_move(word: int) -> dict | None:
    """Decode MOVZ/MOVN/MOVK W/X with architectural width and halfword shift."""
    classes = {
        0x12800000: ("MOVN", 32),
        0x52800000: ("MOVZ", 32),
        0x72800000: ("MOVK", 32),
    ...
    decoded = classes.get(word & 0xFF800000)
    if decoded is None:
        return None
    kind, width_bits = decoded
    halfword = (word >> 21) & 0x3
    if halfword >= width_bits // 16:
        return None
    return {
        "kind": kind,
...
def _wide_move_value(decoded: dict) -> int | None:
    if decoded["kind"] == "MOVK":
        return None
    mask = (1 << decoded["width_bits"]) - 1
    immediate = decoded["imm16"] << decoded["shift"]
    if decoded["kind"] == "MOVZ":
        return immediate & mask
    if decoded["kind"] == "MOVN":
        return (~immediate) & mask
    return None


def _count_exact_wide_paths(images: dict[str, bytes], value: int) -> tuple[dict[str, int], dict[str, int]]:
    """Count exact MOVZ/MOVN sites and adjacent same-register MOVK sequences.

    The first group is genuinely one instruction: its architectural result must
    equal the 32-bit target.  The second group is deliberately narrower than a
    global halfword co-occurrence test: only adjacent MOVZ/MOVN then MOVK words
    with the same width/register are accepted, and the patched result must equal
    the target.
    """
    single = {"movz_w": 0, "movz_x": 0, "movn_w": 0, "movn_x": 0}
    sequence = {
        "movz_then_movk_w": 0,
        "movz_then_movk_x": 0,
        "movn_then_movk_w": 0,
    ...
    for blob in images.values():
        words = [struct.unpack_from("<I", blob, offset)[0] for offset in range(0, len(blob) - 3, 4)]
        decoded = [_decode_wide_move(word) for word in words]
        for item in decoded:
            if item is None or item["kind"] not in ("MOVZ", "MOVN"):
                continue
            if _wide_move_value(item) != value:
                continue
            width = "w" if item["width_bits"] == 32 else "x"
            single[f'{item["kind"].lower()}_{width}'] += 1
        for first, second in zip(decoded, decoded[1:]):
            if (
                first is None
                or second is None
                or first["kind"] not in ("MOVZ", "MOVN")
                or second["kind"] != "MOVK"
                or first["width_bits"] != second["width_bits"]
                or first["rd"] != second["rd"]
            ):
                continue
            base = _wide_move_value(first)
            if base is None:
                continue
            mask = (1 << first["width_bits"]) - 1
            field_mask = 0xFFFF << second["shift"]
            result = (base & ~field_mask) | (second["imm16"] << second["shift"])
            if (result & mask) != value:
                continue
            width = "w" if first["width_bits"] == 32 else "x"
            sequence[f'{first["kind"].lower()}_then_movk_{width}'] += 1
...
def materialisation_audit(images: dict[str, bytes]) -> list[dict]:
    """Audit stored bytes, exact wide-move paths, and logical immediates.

    Stored-word occurrences are byte observations, not instruction paths. Wide
    moves are separated into exact one-instruction MOVZ/MOVN results and a
    narrowly adjacent MOVZ/MOVN-then-MOVK sequence model. Computed values,
    loads, aliases outside the logical-immediate class and other instruction
    orderings remain outside this audit.
    """
    ...
    for value, label in RANKED_LIVE_VALUES.items():
        ...
        wide = _wide_move_halfword_encodings(value)
        wide_counts = {
            key: sum(
        ...
        single_wide, sequence_wide = _count_exact_wide_paths(images, value)
        record["wide_move_single_instruction_sites"] = single_wide
        record["wide_move_two_instruction_sequence_sites"] = sequence_wide
        ...
        stored_total = record["stored_word_total"]
        exact_wide_total = sum(single_wide.values()) + sum(sequence_wide.values())
        orr_sites = record["logical_immediate"]["orr_sites"]
        if stored_total == 0 and exact_wide_total == 0 and orr_sites == 0:
            record["materialisation_conclusion"] = "NO_STORED_WORD_OR_EXACT_MODELED_WIDE_MOVE_OR_ORR_SITE_OBSERVED"
        elif stored_total:
            record["materialisation_conclusion"] = "STORED_WORD_OBSERVED_EXACT_DIRECT_PATHS_REMAIN_MODEL_LIMITED"
        elif exact_wide_total:
            record["materialisation_conclusion"] = "EXACT_WIDE_MOVE_PATH_OBSERVED_OTHER_PATHS_REMAIN_MODEL_LIMITED"
        else:
            record["materialisation_conclusion"] = "EXACT_WIDE_MOVE_PATH_ABSENT_NO_STORED_WORD_ORR_SITE_OBSERVED"
        record["modeled_direct_materialisation_paths_all_absent"] = (
            record["stored_word_total"] == 0
            and exact_wide_total == 0
...
def build_manifest(firmware_dir: Path) -> dict:
    ...
    materialisation_summary = {
        "zero_stored_modeled_direct_paths": sorted(
            row["value"]
            for row in audit
            if row["modeled_direct_materialisation_paths_all_absent"]
        ),
        "stored_word_totals": {
            row["value"]: row["stored_word_total"]
            for row in audit
            if row["stored_word_total"]
        },
        "exact_single_wide_move_site_totals": {
            row["value"]: sum(row["wide_move_single_instruction_sites"].values())
            for row in audit
        },
        "exact_two_instruction_wide_move_site_totals": {
            row["value"]: sum(row["wide_move_two_instruction_sequence_sites"].values())
            for row in audit
        },
        "orr_site_totals": {
            row["value"]: row["logical_immediate"]["orr_sites"] for row in audit
        },
        "derivation": "DERIVED_FROM_EXACT_RANKED_VALUE_MATERIALISATION_ROWS",
    }
    ...
    return {
        ...
        "ranked_value_materialisation_summary": materialisation_summary,
        "ranked_target_reach": "UNKNOWN",
        "claims": {
            "PROVED": [
                "The xbl_config DCB carries candidate address/offset-value pair arrays "
                "with zero terminators under a strict alignment/ascending/terminator "
                "acceptance test.",
                "Both an absolute-address syntactic domain and a base-relative-offset "
                "syntactic domain are present; the latter has no base field in these "
                "arrays. Register-programming semantics and consumers are not established.",
                "No absolute table key reaches any of the four ranked MC bases; this is "
                "an absolute-table-key result only.",
                "The exact materialisation rows derive the precise bounded result: "
                "0x00003333 and 0x00300014 have stored_word_total=0, zero exact "
                "one-instruction MOVZ/MOVN sites, zero exact adjacent MOVZ/MOVN-then-MOVK "
                "sites and zero ORR sites across all nine exact images. 0xc003ffff has "
                "24 stored occurrences and 0x00111111 has 3; computed, loaded, "
                "differently ordered or otherwise unmodeled materialisation remains UNKNOWN.",
            ],
            "SUPPORTED": [
            ...
            "HYPOTHESIS": [
                "A consumer may supply an implicit base to a base-relative array and may "
                "write a ranked address at runtime; this requires a separate consumer and "
                "writer proof.",
            ],
            "REFUTED": [
                "An absolute-address table key in these exact DCB blocks reaches one of the "
                "four ranked MC bases.",
            ],
            "UNKNOWN": [
                ...
                "Register-programming semantics, consumers, the relation to the Experiment "
                "014 GF(2) bank relation, post-boot writability, alias and boundary bypass.",
            ],
        },
    }
