...
def dump_filename(name: str) -> str:
    """Return a deterministic, flat ``.bin`` filename for one blob key.

    The audit tool deliberately discovers only ``*.bin`` files.  Blob keys
    are logical paths and may contain separators or UI-provided punctuation,
    so flatten and restrict them before writing.  The suffix is part of the
    extractor contract, not something callers have to add after the fact.
    """
    safe = re.sub(r"[^A-Za-z0-9_.@+-]", "_", name)
    safe = safe or "blob"
    return safe if safe.endswith(".bin") else safe + ".bin"
...
def main(argv: list[str]) -> int:
    ...
    if args.dump_dir:
        ...
        for name, data in sorted(blobs.items()):
            ...
            if previous is not None and previous != name:
                raise ExtractError(
                    f"blob names collide after sanitization: {previous!r} "
                    f"and {name!r} -> {filename!r}"
                )
            filenames[filename] = name
            destination = out / filename
            if destination.is_symlink():
                raise ExtractError(f"refusing to overwrite symlink {destination}")
            destination.write_bytes(data)
    result["experiment_id"] = "029A-abl-uefi-searchability"
    result["target"] = {
        "model": "SM-A908N",
        "soc": "SM8150",
        "partition": "abl",
        "source_filename": pathlib.Path(args.image).name,
    }
    result["build"] = {
    ...
    result["commands"] = [
        "host: python3 tools/abl_uefi_extract.py --image <private abl image> --dump-dir <scratch> --output <manifest>",
        "host: python3 tools/a90_bank_relation_encoding_audit.py --capture <scratch> --experiment-id 029A-bank-relation-audit-over-abl --source-manifest <029A extraction manifest> --relation-manifest <023R public manifest> --output <029A bank-audit manifest>",
        "host: python3 -m unittest -v tests.test_abl_uefi_extract",
    ]
    result["command_status"] = "REDACTED_REPRODUCTION_TEMPLATE"
    result["command_reason"] = (
        "Private input, scratch and output paths are redacted; exact logical "
        "source and relation dependencies are pinned by their manifests. "
        "These are reproduction templates, not executed-command receipts."
    )
    result["repetitions"] = 1
    result["host_analysis"] = {
        "mode": "HOST_ONLY_READ_ONLY",
        "tool": "tools/abl_uefi_extract.py",
        "tool_sha256": hashlib.sha256(
            pathlib.Path(__file__).resolve().read_bytes()).hexdigest(),
        "extraction_repetitions": 1,
        "dump_contract": "every extracted blob is a flat deterministic *.bin file",
    }
    result["device_binding"] = {
        "status": "NOT_APPLICABLE",
        "reason": "The extractor operates on an already captured image and does not contact or identify a live device.",
    }
    result["rollback"] = {
        "status": "NOT_APPLICABLE",
        "reason": "Host extraction writes only caller-selected scratch files and no device or firmware state.",
    }
    result["recovery"] = {
        "status": "NOT_APPLICABLE",
        "reason": "No device mutation, boot transition or persistent state change occurs during extraction.",
    }
    result["public_private_separation"] = {
        "public": "This manifest contains structure, counts, classifications and SHA-256 digests only.",
        "private": "The source firmware image and extracted *.bin blobs remain private; no raw firmware bytes are published.",
