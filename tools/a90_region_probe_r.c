/* Experiment 023R probe: repaired second-region bank-relation measurement.
 *
 * Experiment 023 measured the GF(2) bank relation in the qseecom carveout and
 * was withheld `NO-GO`.  Four defects were confirmed against
 * tools/a90_dram_timing_probe.c, and this probe repairs all four:
 *
 *   1. Order.  014 alternates relation-first / baseline-first on the low bit
 *      of the repetition index so that the row state one measurement leaves
 *      behind does not bias the next one in a fixed direction.  023 always
 *      measured relation first.
 *   2. Warmup.  014 warms all four reopen forms; 023 warmed two of them, so
 *      two of the four measured forms entered the sample set cold.
 *   3. Barriers.  014's load_barrier is `dsb ld`.  023 used `dsb ld; isb`,
 *      adding a context-synchronisation event inside the timed interval, so
 *      the timed quantity was not the same quantity.
 *   4. Scale.  014 divides the summed pair of reopens by `kept * 2` and so
 *      reports per-reopen milli-ticks.  023 summed two reopens and did not
 *      divide, putting its numbers on a 2x scale.  The two experiments' gaps
 *      were therefore never comparable as printed.
 *
 * The timing core below is copied from 014 verbatim, including the exact
 * inline asm, so that no residual instrumentation difference remains.
 *
 * It also adds what 023 lacked: physical-address provenance.  Three
 * independent sources are emitted rather than assumed --
 *
 *   - /proc/self/pagemap over the mapping, which yields real PFNs if the
 *     kernel populates them for this dma-buf and zeros if it does not;
 *   - the device-tree declared carveout base, as a hypothesis only;
 *   - per-pair deltas tagged with their base offset, which let the analysis
 *     solve for the allocation's offset inside the region behaviourally.
 *
 * The third source is the load-bearing one.  For a linear f a conflict is
 * f(PA_a ^ PA_b) == 0.  With allocation base B and page-aligned offsets a and
 * b = a ^ d, PA_a ^ PA_b equals d only when adding B introduces no differing
 * carries.  If B is aligned to the allocation size that holds for every a; if
 * B is offset by some delta it fails for a predictable subset of a.  Sweeping
 * many a for a difference already proved kernel-class by 014 therefore
 * measures the alignment instead of assuming it, and uses only bits 13..23,
 * never PA24, so it cannot be circular with the PA24 claim under test.
 *
...
#define PROBE_SCHEMA "a90_region_probe_r_v1"
    ...
    for (int arg = difference_start; arg < argc && arg - difference_start < (int)MAX_DIFFERENCES; ++arg) {
        ...
        for (uint64_t index = 0; index < pairs; ++index) {
            ...
            printf("{\"schema\":\"%s\",\"type\":\"pair\",\"value\":\"0x%llx\","
                   "\"offset\":\"0x%llx\",\"delta\":%lld}\n",
                   PROBE_SCHEMA, (unsigned long long)difference,
                   (unsigned long long)a, (long long)delta);
        }
        if (used == 0) {
            printf("{\"schema\":\"%s\",\"type\":\"difference\",\"value\":\"0x%llx\","
                   "\"status\":\"NO_PAIRS\"}\n",
