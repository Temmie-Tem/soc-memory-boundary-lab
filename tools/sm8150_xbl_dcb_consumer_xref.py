#!/usr/bin/env python3
"""Host-only Experiment 020 cross-reference of XBL DCB consumers and a largest-RWE candidate.
...
model returned no target match. Experiment 019 then showed candidate
base-relative DCB pair arrays; a consumer would write through a register offset
rather than an immediate one, but that consumer link remains UNKNOWN.
...
loader and its specific section-directory consumers, and audits a largest-RWE
candidate segment's pointer-like globals without assigning it a controller
identity.

The finding is bounded: the candidate segment's possible bases are not constants
in this image's supported static model. Values arrive as function arguments and
some candidate-segment globals are zero-initialised. General walkers, taint
through MOV/ORR/shift/extend/mask/load operations, and other writer paths remain
UNKNOWN; this complements but does not refute Experiment 018's scoped result.
...
SCHEMA = "sm8150-xbl-dcb-consumer-xref-v2"
...
# One candidate-segment store setter, pinned after reading its disassembly rather
# than picked by a heuristic: it writes five candidate-segment globals and is
# reached by one direct BL.
PINNED_SETTER_VA = 0x9FC06410
PINNED_SETTER_END = 0x9FC0643C

KNOWN_FALSE_NEGATIVE_START = 0x148689C8
KNOWN_FALSE_NEGATIVE_END = 0x14868A60
KNOWN_FALSE_NEGATIVE_STORE = 0x14868A50
KNOWN_FALSE_NEGATIVE_FILE_OFFSET = 0x4FA50
KNOWN_FALSE_NEGATIVE_SHA256 = "02248b786ffb501a5fa9242aa3952e1e4d783f47464952e96ca2704a9f94341e"

# Every word below is load-bearing for the exact six-byte-record offset/value
# shape. The range digest independently binds the intervening bytes and the
# back edge; these individual pins make failures local and reviewable.
KNOWN_FALSE_NEGATIVE_WORDS = {
...
def section_directory_readers(image: Image) -> list[dict]:
    """Search for the consecutive-directory-read pattern used by this scan."""
...
def table_walker_analysis(image: Image, loop_sites: list[dict]) -> dict:
    ...
    * a body containing ``RET`` is a function epilogue restoring callee-saved
    ...
    return {
        "model_scope": "NARROW_DIRECT_LAST_DEFINITION_WITHIN_OBSERVED_LOOP_BODY",
        ...
        "zero_within_stated_model": len(walkers) == 0,
        "general_walker_presence": "UNKNOWN",
        "model_limitations": [
            "DOES_NOT_TRACK_MOV_OR_REGISTER_COPY_TAINT",
            "DOES_NOT_TRACK_ORR_SHIFT_EXTEND_MASK_TAINT",
            "DOES_NOT_TRACK_LOAD_TAINT_AFTER_REGISTER_COPY",
            "LOOP_WINDOW_AND_DIRECT_BRANCH_SHAPE_ONLY",
        ],
    }


def known_narrow_model_false_negative(image: Image) -> dict:
    """Pin one exact loop that the narrow classifier misclassifies.

    The loop uses UMADDL and load/copy taint before its register-offset store.
    Those semantics are intentionally outside this stage's model, so this is a
    bounded false-negative record and a next-stage candidate, not a DCB-writer
    proof.
    """
    start, end = KNOWN_FALSE_NEGATIVE_START, KNOWN_FALSE_NEGATIVE_END
    file_offset = image.file_offset(start)
    if (
        file_offset != 0x4F9C8
        or image.file_offset(KNOWN_FALSE_NEGATIVE_STORE) != KNOWN_FALSE_NEGATIVE_FILE_OFFSET
        or image.file_offset(end - 4) != file_offset + (end - start) - 4
    ):
        raise XrefError("known false-negative mapping mismatch")
    for va, expected in KNOWN_FALSE_NEGATIVE_WORDS.items():
        if image.word(va) != expected:
            raise XrefError(f"known false-negative instruction mismatch at 0x{va:x}")
    digest = sha256(image.data[file_offset : file_offset + end - start])
    if digest != KNOWN_FALSE_NEGATIVE_SHA256:
        raise XrefError("known false-negative range hash mismatch")
    return {
        "status": "DISCOVERED_FALSE_NEGATIVE_TO_NARROW_MODEL",
        "range": {
        ...
        "loop_shape": "BACKWARD_BRANCH_LOOP",
        "store_va": f"0x{KNOWN_FALSE_NEGATIVE_STORE:08x}",
        "narrow_classifier_result": "LOADED_LOOP_INVARIANT",
        "pinned_dataflow_shape": "PREHEADER_W9_ZERO;PREHEADER_W10_SIX;B_TO_LOOP;UMADDL_INDEX;LDRH_OFFSET;LDRB_VALUE;LDR_BASE;STR_REGISTER_OFFSET;ADD_INDUCTION;CMP;B_CC_BACK_EDGE",
        "record_shape": "DIRECT_SIX_BYTE_RECORD_OFFSET_VALUE_STORE_SHAPE",
        "record_semantics": "UNKNOWN",
        "unmodeled_reason": "UMADDL_AND_LOAD_COPY_TAINT_NOT_TRACKED",
        "dcb_or_controller_semantics": "UNKNOWN",
        "runtime_base_and_execution": "UNKNOWN",
        "next_stage": "MOV_SHIFT_EXTEND_LOAD_AWARE_CFG_DATAFLOW",
    }
...
def aop_controller_reference_audit(firmware_dir: Path) -> dict:
    ...
    return {
        ...
        "elf_class": 32,
        "machine": machine,
        "literal_scan_scope": "ALIGNED_U32_WORDS_IN_FILE_BACKED_PT_LOADS",
        "thumb2_movw_movt_or_computed_constructions": "UNKNOWN",
        "load_segments": [
...
def largest_rwe_candidate_segment(image: Image) -> dict:
    """Report the largest RWE segment without assigning runtime identity."""
    ...
    return {
        "found": True,
        "identity_confidence": "SUPPORTED_LARGEST_RWE_SIZE_AND_CONTENT_CANDIDATE",
...
def candidate_segment_global_store_census(image: Image, segment_vaddr: int) -> dict:
    """Count stores into candidate-segment globals, split by source.
...
def pinned_candidate_segment_store_setter(image: Image) -> dict:
    ...
    value from an incoming argument register, this bounded view cannot resolve
    the value reaching the candidate segment.
    """
    stores = []
    pages: dict[int, int] = {}
...
def build_manifest(firmware_dir: Path) -> dict:
    ...
    candidate_segment = largest_rwe_candidate_segment(image)
    setter = pinned_candidate_segment_store_setter(image)
    globals_census = (
        candidate_segment_global_store_census(image, int(candidate_segment["vaddr"], 16))
        if candidate_segment["found"]
        else {}
    )
    known_false_negative = known_narrow_model_false_negative(image)
    ...
    return {
        "schema": SCHEMA,
        "experiment_id": "020-xbl-dcb-consumer-xref",
        ...
        "known_narrow_model_false_negative": known_false_negative,
        ...
        "largest_rwe_candidate_segment": candidate_segment,
        "candidate_segment_global_store_census": globals_census,
        "pinned_candidate_segment_store_setter": setter,
        "pinned_setter_direct_callers": setter_callers,
        "next_discriminator": "MOV_SHIFT_EXTEND_LOAD_AWARE_CFG_DATAFLOW_OVER_67_REGISTER_OFFSET_LOOPS_AND_8_COMPUTED_ADDRESS_SITES_THEN_CALLER_ORIGIN_0x9fc023f0_AND_SHRM_BASE_COMPUTED_DISPLACEMENT",
        "claims": {
            "PROVED": [
                ...
                "The exact largest-RWE segment census records no file-backed 64-bit "
                "value inside the SoC control aperture, and none equal to a ranked MC "
                "base.",
                "The exact pinned global stores write zero or incoming function-argument "
                "values into the candidate segment's globals, not static constants.",
                "The narrow classifier reports zero register-offset table walkers "
                "within the stated direct-last-definition loop model; its exact pinned "
                "false negative invalidates any general zero-walker absence claim.",
                "The exact XBL has a pinned narrow-model false negative at store "
                "0x14868a50 in the 0x148689dc..0x14868a60 loop: the exact bytes pin a "
                "direct six-byte-record offset/value store shape that this narrow model "
                "misses; full record semantics, DCB identity, runtime base and execution "
                "remain UNKNOWN and are reserved for Experiment 024.",
                "The same narrow classifier reports zero for the computed-address idiom, "
                "ADD Xd,Xn,Xm feeding STR Wt,[Xd]; MOV/shift/extend/mask/load taint is "
                "not modeled, so this is not a general absence claim.",
                "AOP is ELF32 ARM. Its aligned-U32 literal scan contains no literal equal "
                "to a ranked MC base, while Thumb-2 MOVW/MOVT and computed constructions "
                "remain UNKNOWN; its only DCB directory-read pairs are isolated single "
                "sections without the consecutive-index run that separates a real dispatcher from "
                "coincidence.",
            ],
            "SUPPORTED": [
                "The largest RWE segment is a candidate segment selected by size and "
                "content, not a proved controller identity; its exact candidate-segment "
                "stores and runtime-argument setter observations are independently pinned.",
                "Experiment 020 complements Experiment 018 by covering register-offset "
                "and computed-address shapes outside its immediate-offset models; it "
                "does not refute or prove writer absence.",
            ],
            "HYPOTHESIS": [
                "A MOV/shift/extend/load-aware CFG/dataflow pass over the 67 loops and 8 "
                "computed-address sites may recover a DCB consumer; caller origin at "
                "0x9fc023f0 and SHRM-base computed displacement are follow-on questions.",
            ],
            "UNKNOWN": [
                "The runtime origin of the candidate-segment arguments, and therefore "
                "the absolute addresses any candidate-segment store reaches.",
                "Whether a DCB table walker exists outside this loop model -- one whose "
                "back edge lies further than the search window, branches indirectly, "
                "tests before it stores, or copies a loaded index through MOV/ORR/shift/"
                "extend/mask/load taint -- and whether any consumer of the DCB base-relative "
                "tables lives outside the exact XBL at all.",
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    write_no_clobber(args.output, (json.dumps(manifest, indent=1, sort_keys=True) + "\n").encode("utf-8"))
