...
model returned no target match.  Experiment 019 then showed the boot chain
programs registers from base-relative DCB data, so a consumer writes through a
register offset rather than an immediate one.
...
The finding that matters is negative in a specific way: the DDR driver's bases
are never constants in the image.  They arrive as function arguments and live in
zero-initialised globals, so no constant-terminating static model can resolve a
controller store target, and Experiment 018's result is structurally guaranteed
rather than evidence about whether a writer exists.
...
def section_directory_readers(image: Image) -> list[dict]:
    """Every DCB consumer must read a directory slot as an LDRH pair."""
...
def ddr_driver_segment(image: Image) -> dict:
    """The largest RWE segment is the DDR driver; report its constant content."""
    ...
    return {
        ...
        "claims": {
            "PROVED": [
                ...
                "The DDR driver segment contains no file-backed 64-bit value inside the SoC "
            ...
            "REFUTED": [
                ...
                "exists. Its four models all terminate in a constant, and the base values "
                "are not constants in this image, so the negative is structurally "
                "guaranteed for those models.",
            ],
            "UNKNOWN": [
