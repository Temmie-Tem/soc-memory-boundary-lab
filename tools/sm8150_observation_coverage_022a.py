...
EXPERIMENT_ID = "022A-observation-coverage"
...
def counterexample_provenance(manifest_dir: Path = MANIFEST_DIR) -> dict[str, str]:
    ...
    if not path.is_file():
        raise CoverageError(
            f"missing counterexample source manifest: {COUNTEREXAMPLE_SOURCE_MANIFEST}"
        )
    actual = sha256(path.read_bytes())
    expected = DEPENDENCY_MANIFESTS[COUNTEREXAMPLE_SOURCE_MANIFEST]
    if actual != expected:
        raise CoverageError(f"counterexample source manifest hash mismatch: {actual}")
    try:
...
def static_definition_of_done() -> dict:
    ...
    return {
        ...
        "commands": {
            ...
            "command": "python3 tools/sm8150_observation_coverage_022a.py --output <public-manifest-path>",
            "reason": (
                "The default private firmware/SHRM inputs and output path are "
                "redacted; exact inputs are pinned by their published hashes. "
...
def build_manifest(firmware_dir: Path, dump_path: Path) -> dict:
    ...
    return {
        ...
        "claim_scope": (
            "Address counts and negatives are limited to the two enumerated "
            "channels; the known counterexample refutes completeness and no "
