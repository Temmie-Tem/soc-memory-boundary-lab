#!/usr/bin/env python3
"""Audit exact A90 firmware and live registers for the pinned bank relation.

Experiment 014 recovered a three-row GF(2) bank basis and
`tools/a90_bank_hash_literal_audit.py` searched firmware for it.  That audit
had two defects that this one repairs.

Its target was wrong.  It searched the row space of the Experiment 014 basis,
which omitted PA24.  Experiment 023R resolved PA24's contribution and pinned
the kernel uniquely, and the corrected row space shares only three of its seven
covectors with the set that audit used: four of its seven targets were values
the relation never had, and four real ones were never searched.
...
Why the row space is the right target.  A conflict measurement determines
...
#: Experiment 023R's relation.  PA24 contributes 0b110, so it joins b1 and b2.
...
def audit(targets: dict[str, bytes], registers: list[dict],
    ...
    return {
        ...
        "nonzero_register_count": sum(1 for r in registers
                                      if int(r["value"], 16) != 0),
        ...
        "adjacent_triple": adjacent_triple_audit(targets, masks),
