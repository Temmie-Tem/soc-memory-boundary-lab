#!/usr/bin/env python3
"""Host-only Experiment 032: bounded arithmetic frontier extension.

This experiment is deliberately a narrow v3 extension of the frozen
Experiment 031 transform.  It consumes the exact 031 public result (which
pins the 027/029 source chain), hash-checks every dependency before importing
the 031-compatible decoder, and visits only the same 71 027
``INDIRECT_OR_UNSUPPORTED`` site ranges.  It retains every 031 scalar and
``DIRECT_CONTROL_DISPATCH_REPAIR`` rule exactly, then adds only:
...
* destination-local conservative kills for the 029
  ``TAINT_KILL_REQUIRED`` scalar families, and
* source-qualified ``MADD``/``UMADDL`` and shifted ``EOR``/``BIC``.

Pair/sign-extending memory, system/control, indirect aliases, malformed, and
otherwise unknown forms remain fail-closed.  Arithmetic results are
destination-local: an unknown result kills only its destination and never
turns into an absence claim.
...
SCHEMA = "sm8150-dcb-arithmetic-frontier-v3"
EXPERIMENT_ID = "032-dcb-arithmetic-frontier"
...
PREDICTED_IMPLEMENTED_FAMILIES = frozenset(
    V2_PREDICTED_IMPLEMENTED_FAMILIES | ARITHMETIC_PREDICTED_IMPLEMENTED_FAMILIES
)
...
def _all_range_forms_selected(manifest_029: Mapping[str, Any]) -> dict[str, Any]:
    ...
    return {"all_range_forms_selected_site_count": len(eligible), "total_fail_closed_sites": 71, "ratio": f"{len(eligible)}/71", "site_indices": eligible, "basis": "syntactic membership only; neither CFG reachability nor closure prediction; 029 frontier families are limited to the exact 031 scalar domain plus MADD/UMADDL, EOR_SHIFT, and BIC_SHIFT"}
...
def _admit_reached_extension_events(events: Sequence[Mapping[str, Any]], lookup: Mapping[tuple[int, str, str], Mapping[str, str]]) -> dict[str, int]:
    ...
    for event in events:
        ...
        if admitted is None:
            outside += 1
            raise ExtensionError(f"reached v3 extension event is outside selected 029 frontier: {key}")
        ...
        if mapped_family != admitted["family"]:
            family_mismatch += 1
            raise ExtensionError(f"reached v3 family mismatch at {key}: {mapped_family} != {admitted['family']}")
        # 029 intentionally classified the three-source residual as UNKNOWN
        # before primary-source review.  A v3 event may therefore carry the
        ...
        if not label_compatible:
            label_mismatch += 1
            raise ExtensionError(f"reached v3 label mismatch at {key}: {event['label']} != {admitted['label']}")
...
def _canonical_records(records: Sequence[Mapping[str, Any]]) -> list[str]:
    """Canonical full-record JSON used for inherited 031 equivalence."""
...
def assert_inherited_031_equivalence(
    ...
    if expected_canonical != actual_canonical:
        raise ExtensionError(f"inherited 031 {label} full-record equivalence mismatch")
...
def analyze_exact_sites(decoder: Any, manifest_031: Mapping[str, Any], manifest_029: Mapping[str, Any], firmware: bytes) -> dict[str, Any]:
    if len(firmware) != XBL_SIZE or sha256(firmware) != XBL_SHA256:
        raise ExtensionError("exact XBL input mismatch")
    _validate_031_manifest(manifest_031); _validate_029_manifest(manifest_029)
    image = decoder.Image(firmware)
    # The committed 031 result embeds the exact 027 site row under ``site``;
    # use that frozen range identity while retaining the 031 top-level row for
    # the per-site transition ledger.
    rows = []
    baseline_031_by_store: dict[str, Mapping[str, Any]] = {}
    for row in manifest_031["sites"]:
        if not isinstance(row, Mapping) or not isinstance(row.get("site"), Mapping):
            raise ExtensionError("Experiment 031 embedded site row is malformed")
        baseline_031_by_store[str(row["store_va"])] = row
        embedded = row["site"]
    ...
    for index, site in enumerate(rows):
        ...
        result["v3_handled_forms"] = result["v2_handled_forms"]
        ...
        result["delta_vs_027"] = {"was_fail_closed": True, "is_fail_closed": still_fail_closed, "transitioned_to_no_target_within_v3_model": not still_fail_closed, "transition": "027_INDIRECT_OR_UNSUPPORTED_TO_032_INDIRECT_OR_UNSUPPORTED" if still_fail_closed else "027_INDIRECT_OR_UNSUPPORTED_TO_032_NO_TARGET_WITHIN_MODEL", "baseline_discriminators": list(site["discriminators"])}
        baseline = baseline_031_by_store.get(str(site["store_va"]))
        if baseline is None:
            raise ExtensionError(f"031 site identity mismatch at {site['store_va']}")
        ...
        result["delta_vs_031"] = {
            ...
            "transitioned_to_no_target_within_v3_model": baseline_fail_closed and not still_fail_closed,
            "transition": f"031_{baseline_state}_TO_032_{current_state}",
            "baseline_discriminators": baseline_discriminators,
        }
        results.append(result)
    results.sort(key=lambda row: parse_hex(row["store_va"], "result store"))
    admission_events = [event for row in results for event in row["v3_handled_forms"]]
    ...
    extension_admission = {
        ...
        **admission_mismatches,
        ...
        "exact_selected_identity": len(admission_lookup) == 298,
    }
    counts: dict[str, int] = {}
    ...
    for row in results:
        for label in row["discriminators"]:
            counts[label] = counts.get(label, 0) + 1
        for item in row["v3_handled_forms"]:
    ...
    reached_extension_events = unique_events(
        (event for row in results for event in row["v3_handled_forms"]),
    ...
    inherited_extension_events = [
        event for event in reached_extension_events
        if event.get("family") in V2_PREDICTED_IMPLEMENTED_FAMILIES
    ]
    inherited_expected_events = list(manifest_031["analysis"]["reached_extension_events"])
    inherited_extension_equivalence = assert_inherited_031_equivalence(
        inherited_expected_events,
        inherited_extension_events,
        "reached_extension_events",
    )
    inherited_expected_control = unique_events(
        (event for row in manifest_031["sites"] for event in row.get("control_flow_events", [])),
        ("site_index", "va", "raw_word", "family", "effect"),
    )
    inherited_control_equivalence = assert_inherited_031_equivalence(
        inherited_expected_control,
        direct_control_events,
        "direct_control_events",
    )
    inherited_taint_equivalence = assert_inherited_031_equivalence(
        manifest_031["analysis"]["reached_taint_kill_events"],
        reached_taint_kill_events,
        "reached_taint_kill_events",
    )
    inherited_031_equivalence = {
    ...
    direct_control_repair = {
        ...
        "outcome_split": {
            "with_direct_control_repair": {"site_count": len(with_control), "no_target_within_v3_model": sum("NO_TARGET_WITHIN_MODEL" in row["discriminators"] for row in with_control), "fail_closed": sum(INDIRECT in row["discriminators"] for row in with_control)},
            "without_direct_control_repair": {"site_count": len(without_control), "no_target_within_v3_model": sum("NO_TARGET_WITHIN_MODEL" in row["discriminators"] for row in without_control), "fail_closed": sum(INDIRECT in row["discriminators"] for row in without_control)},
        },
    ...
    actual_closed = [row for row in results if row["delta_vs_027"]["transitioned_to_no_target_within_v3_model"]]
    actual_closed_vs_031 = [row for row in results if row["delta_vs_031"]["transitioned_to_no_target_within_v3_model"]]
    transition_quadrants = validate_031_to_032_transition_quadrants(results)
    ...
    return {
        ...
        "actual_delta_vs_027": {"baseline_fail_closed_sites": 71, "v3_fail_closed_sites": len(residual_sites), "sites_remaining_fail_closed": len(residual_sites), "sites_transitioned_to_no_target_within_v3_model": len(actual_closed), "transition_identity": len(residual_sites) + len(actual_closed) == 71, "transition_scope": "V3_MODEL_ONLY", "combined_scalar_plus_dispatch_v3": True, "dispatch_dependency": "DIRECT_CONTROL_DISPATCH_REPAIR", "absence_claim": "NO_ABSENCE_CLAIM"},
        "actual_delta_vs_031": {"baseline_fail_closed_sites": 20, "v3_fail_closed_sites": len(residual_sites), "sites_remaining_fail_closed": len(residual_sites), "sites_transitioned_to_no_target_within_v3_model": len(actual_closed_vs_031), "transition_identity": len(residual_sites) + len(actual_closed_vs_031) == 20, "transition_scope": "V3_MODEL_ONLY", "absence_claim": "NO_ABSENCE_CLAIM"},
        ...
        "remaining_unsupported_sites": [{"site_index": row["site_index"], "store_va": row["store_va"], "families": row["v3_syntactic_frontier"]["families"], "unsupported_forms": row["unsupported_forms"]} for row in residual_sites],
        ...
        "remaining_syntactic_frontier": {"occurrence_count": len(residual_range_forms), "unique_va_count": len(residual_unique), "unique_raw_word_count": len(residual_words), "site_count": len({int(item["site_index"]) for item in residual_range_forms}), "classes": sorted(residual_range_counts), "identity": f"{len(residual_range_forms)} occurrences / {len(residual_unique)} unique VAs / {len(residual_words)} unique raw words across {len({int(item['site_index']) for item in residual_range_forms})} residual sites; range membership is not CFG reachability"},
        ...
        "inherited_031_equivalence": inherited_031_equivalence,
...
def definition_of_done() -> dict[str, Any]:
    ...
    return {
        ...
        "precondition": {"status": "PROVED", "device_access": "none", "exact": "Exact XBL plus exact-hash-pinned 031/029/027 source/public inputs; no device action."},
        "result": {"classification": "CLASS C (TRANSFORM ONLY)", "status": "BOUNDED_ARITHMETIC_AND_DIRECT_CONTROL_EXTENSION_UNKNOWN_GLOBAL", "proved": "Exact input identity, source/manifest pins, inherited 031 scalar/direct-control semantics, qualified arithmetic masks, bounded CFG, and deterministic accounting.", "supported": "Only the explicitly qualified 031 forms plus MADD/UMADDL modulo-width arithmetic, shifted EOR/BIC, and bounded direct-control dispatch repair.", "unknown": "Runtime aliases, current destinations, unsupported forms, execution/order, and global consumer/writer presence remain UNKNOWN."},
        ...
        "deterministic_repetition_validation": {"repetition_count": 2, "repetition_unit": "fresh host manifest generations", "repetition_result": "BYTE_IDENTICAL", "validation_checks": ["exact XBL and 031/029/027 source/public hashes", "031 semantic boundary and exact 71-site identity", "qualified Arm-source MADD/UMADDL/EOR/BIC masks and modulo-width semantics", "pre-state overlap and Rd=31 discard controls", "DIRECT_CONTROL_DISPATCH_REPAIR inherited event/site/family accounting", "residual pair/sign-extending/system/indirect accounting", "O_EXCL/O_NOFOLLOW mode0644 publication"]},
        "tool_and_build": {"tool": "sm8150_dcb_arithmetic_frontier.py", "build": "NOT_APPLICABLE", "build_reason": "Interpreted host transform; no firmware/kernel build."},
    }
...
def build_manifest(
    ...
    decoder, dep031, dep029, hashes = load_pinned_dependencies(
    ...
    analysis = analyze_exact_sites(decoder, dep031, dep029, firmware)
    return {
        "schema": SCHEMA,
        ...
        "scope": {"source_experiment": "031-dcb-scalar-frontier-extension", "frontier_source": "029-dcb-unsupported-frontier", "fail_closed_site_count": 71, "scan_mode": "BOUNDED_CFG_DATAFLOW_ONLY", "cfg_reachability": "ANALYZED_WITHIN_SITE_RANGE_ONLY", "arbitrary_range_scan": False, "device_mmio_write": False, "global_writer_absence": "UNKNOWN", "writer_absence_claim": False, "scalar_admission": "EXACT_031_SELECTED_283_PLUS_032_ARITHMETIC_15", "arithmetic_admission": "MADD_UMADDL_EOR_SHIFT_BIC_SHIFT", "direct_control_dispatch_repair": "DIRECT_CONTROL_DISPATCH_REPAIR_INHERITED", "combined_result_scope": "V3_MODEL_ONLY_NO_ABSENCE_CLAIM", "preserved_fail_closed_families": ["PAIR_MEMORY", "SIGN_EXTENDING_MEMORY", "SYSTEM_CONTROL", "INDIRECT_BRANCH_TARGET_OR_RUNTIME_ALIAS", "UNKNOWN_FORM"]},
        ...
        "dependency_031": {"classification": dep031["classification"], "experiment_id": dep031["experiment_id"], "schema": dep031.get("schema"), "site_count": dep031["analysis"]["site_count"], "fail_closed_site_count": dep031["analysis"]["discriminator_counts"][INDIRECT], "writer_absence": dep031["analysis"]["writer_absence"], "writer_absence_claim": dep031["analysis"]["writer_absence_claim"], "transitioned_to_no_target_within_v2_model": dep031["analysis"]["actual_delta_vs_027"]["sites_transitioned_to_no_target_within_v2_model"]},
        "dependency_027": {"classification": "PINNED_THROUGH_031", "site_count": 73, "fail_closed_site_count": 71, "writer_absence": "UNKNOWN", "writer_absence_claim": False},
        ...
        "claims": {"PROVED": ["Exact XBL and 031/029/027 source and manifest hashes were validated before importing pinned dependencies.", "Exactly the same 71 027 fail-closed site identities and inherited 031 scalar/direct-control semantics were analyzed within their bounded ranges.", "Only source-qualified MADD/UMADDL modulo-width arithmetic, shifted EOR/BIC identities, and the complete 031 scalar frontier are applied.", "DIRECT_CONTROL_DISPATCH_REPAIR is inherited as a separately bounded direct-CFG repair with no runtime target authority; its exact event/site/family counts remain published.", "Pair/sign-extending memory, system/control, indirect branches, reserved encodings, and unknown forms remain fail-closed."], "SUPPORTED": ["Any reported DCB consumer or MC/SHRM path is bounded symbolic evidence only; current_destination remains UNKNOWN."], "HYPOTHESIS": ["The all_range_forms_selected ratio and arithmetic syntactic counts are range-membership facts only; they are neither CFG reachability nor a closure prediction.", "Exact pointer-preserving arithmetic identities are limited to explicit mathematically exact cases."], "REFUTED": [], "UNKNOWN": ["Exact remaining unsupported sites/forms are not reachability-to-runtime claims.", "Unknown arithmetic results are destination-local UNKNOWN values, not absence evidence.", "No unsupported form is converted into a writer/consumer absence claim.", "DIRECT_CONTROL_DISPATCH_REPAIR does not establish runtime branch targets, execution order, device authority, or an absence claim.", "Runtime aliases, current destinations, protected-memory semantics, and global DCB consumer/writer presence remain UNKNOWN."]},
        ...
        "reproducible_command_template": "python3 tools/sm8150_dcb_arithmetic_frontier.py --firmware-dir <exact-firmware-dir> --manifest-dir <public-manifest-dir> --output <public-manifest-path>",
    }
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    manifest = build_manifest(args.firmware_dir, args.manifest_dir, args.tool_031, args.manifest_031, args.tool_029, args.manifest_029, args.tool_027, args.manifest_027)
